...
export class ReceptionService {
  ...
  public getValidMileage(chassis: string, milage: number): Observable<any[]> {
    return this.apiService.get<any[]>(
      `WorkshopReception/MileageValidation?chassis=${chassis}&mileage=${milage}`
    );
  }
  ...
  public postInsideImage(imgList: any) {
    ...
    if (Array.isArray(imgList) && imgList.length > 0) {
      ...
      return this.apiService.post<any>(
        `General/VehiclePartInteriorImage`,
        galleryData
      );
    }
  ...
  public postBackPartImage(imgList: any) {
    const galleryData: FormData = this.buildFormDataImages(
      imgList
      // 'backPartImage'
    );
    return this.apiService.post<any>(
      `General/VehicleBackPartImage`,
      galleryData
    );
  }
  ...
  buildFormDataImages(imgList: any): FormData {
    ...
    for (let i = 0; i < imgList.length; i++) {
      galleryData.append(
        `ImageFileRanges[${i}].numberOfAppointment`,
        imgList[i].appointmentNumber
      );
      galleryData.append(
        `ImageFileRanges[${i}].description`,
        imgList[i].comment
      );
      galleryData.append(
        `ImageFileRanges[${i}].data`,
        `data:image/png;base64,${imgList[i].image}`
      );
