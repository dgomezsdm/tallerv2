...
export class HomePage implements OnInit {
  appointments: AppointmentPool[] = [];
  filteredAppointments: AppointmentPool[] = [];
  allAppointments: AppointmentPool[] = [];
  loading = false;
    ...
    this.appointmentService
      ...
      .subscribe({
        next: (res) => {
          ...
          this.allAppointments = [...this.appointments]; // ✅ Actualizar backup
          this.filteredAppointments = [...this.appointments]; // ✅ Actualizar filtradas
  ...
  onSearch(term: string) {
    ...
    this.filteredAppointments = this.allAppointments.filter((ap) => {
      return (
        // Buscar por nombre
        ap.clientNames?.toLowerCase().includes(searchLower) ||
        // Buscar por apellido
        ...
        `${ap.clientNames} ${ap.clientSurnames}`
          .toLowerCase()
          .includes(searchLower) ||
        // Buscar por número de cita
        ap.numberOfAppointment?.toLowerCase().includes(searchLower) ||
        // Buscar por chassis
        ap.chassis?.toLowerCase().includes(searchLower) ||
        // Buscar por teléfono
        ap.clientPhone?.includes(term) ||
        // Buscar por marca
        ap.brandDescription?.toLowerCase().includes(searchLower) ||
        // Buscar por modelo
        ap.modelDescription?.toLowerCase().includes(searchLower) ||
        // Buscar por tipo de cita
        ap.typeDescription?.toLowerCase().includes(searchLower)
      );
    });

    console.log(
      `Encontradas ${this.filteredAppointments.length} de ${this.allAppointments.length} citas`
    );
  }
